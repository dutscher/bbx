<script lang="ts">
    ...
    storedActiveProduct.subscribe(store => {
        ...
        if (
            activeProduct &&
            ((activeProduct.id !== product.id && activeProduct.type === type) || activeProduct.id === 0)
        ) {
            showTooltip = false;
        }
    });
    ...
    const getTitle = product => {
        ...
        const isStarTrekFilterOnly =
            activeTagsIds && activeTagsIds.includes(ID_STAR_TREK) && activeTagsIds.length === 1;
